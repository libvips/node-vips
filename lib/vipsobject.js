...
module.exports = function (vips) {
    ...
    VipsObject.prototype.set_from_string = function (string_options) {
        var result = libvips.vips_object_set_from_string(this.buffer, 
            string_options);

        if (result != 0) {
            throw new vips.VipsError('unable to set options ' + string_options);
        }
    }
