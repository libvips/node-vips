...
module.exports = function (vips) {
    ...
    Operation.prototype.get_args = function () {
        ...
        var argument_map_callback = ffi.Callback('pointer',
            ...
            function (vo, pspec, argument_class) {
    ...
    function call_args() {
        ...
        if (supplied_arguments.length === n_required + 1) {
            var options = supplied_arguments.pop();

            if (options !== undefined) {
                supplied_optional_arguments = options;
            }
        }
        ...
        if ('string_options' in supplied_optional_arguments) {
            var options = supplied_optional_arguments['string_options'];
        ...
        if (vop.isNull()) {
            throw new vips.VipsError('unable to call ' + operation_name);
        }
    ...
    function generate_wrappers() {
        ...
        function add_docs(gtype) {
            ...
            if (banned.indexOf(nickname) === -1) {
                try {
                    wrapper_of[nickname] = generate_wrapper(nickname);
                }
                catch (err) {
                }
