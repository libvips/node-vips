...
module.exports = function (vips) {
  ...
  function callArgs () {
    ...
    // pull out the special 'async' option
    var async = null;
    if ('async' in suppliedOptionalArgs) {
      async = suppliedOptionalArgs['async'];
    ...
    function handleBuildReturn (err, vop) {
      console.log("build return for " + nickname + " ...");
      ...
      if (vop.isNull()) {
        return async(new vips.VipsError('unable to call ' + nickname), null);
      }
      ...
      return async(null, result);
    }

    // build operation
    if (async) {
      result = libvips.vips_cache_operation_build.async(op.buffer, handleBuildReturn);
      console.log("back from build.async, result = " + 
