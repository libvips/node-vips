...
module.exports = function (vips) {
    ...
    Image.new_from_file = function (vips_filename, options) {
        ...
        var filename = vips_lib.vips_filename_get_filename(vips_filename);
        var string_options = vips_lib.vips_filename_get_options(vips_filename);
        var name = vips_lib.vips_foreign_find_load(filename);
        if (name.isNull()) {
    ...
    Image.prototype.write_to_file = function (vips_filename, options) {
        ...
        var filename = vips_lib.vips_filename_get_filename(vips_filename);
        var string_options = vips_lib.vips_filename_get_options(vips_filename);
        var name = vips_lib.vips_foreign_find_save(filename);
        if (name.isNull()) {
