...
module.exports = function (vips) {
    ...
    Image.new_from_array = function (array, scale, offset) {
        ...
        var n = width * height;
        var a = ref.alloc(DoubleP, n);

        for (var y = 0; y < height; y++) {
            for (var x = 0; y < width; y++) {
        ...
        image.set_type(vips.GTYPES.gdouble, 'scale', scale);
        image.set_type(vips.GTYPES.gdouble, 'offset', offset);
    ...
    Image.prototype.get_fields = function () {
        var array = libvips.vips_image_get_fields(this.buffer);
        var names = [];
        var i = 0;
        while (array[i] != ref.NULL) { 
            var name = array[i];
            names.push(name);
            libglib.g_free(array[i]);
            i += 1
        }
        libglib.g_free(array)
