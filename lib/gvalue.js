...
module.exports = function (vips) {
    ...
    // debugging: track the number of g_free callbacks we are waiting for
    vips.n_g_free = 0;

    // g_free() as a callback we can pass to funcs ... sadly it seems we have
    // to bounce via JS, we can't just use a C func
    var g_free = ffi.Callback('void', ['pointer'], (p) => {
        console.log("GValue.g_free callback");
        libglib.g_free(p);
        vips.n_g_free -= 1;
    });
    ...
    GValue.prototype.get = function () {
        ...
        switch (gtype) {
            ...
            case GTYPES.VipsBlob:
                ...
                var ptr = libvips.vips_value_get_blob(this.buffer.ref(), 
                    size_buf);
            ...
            default:
                switch (fundamental) {
                    case GTYPES.GEnum:
                        var enum_value = 
                            libgobject.g_value_get_enum(this.buffer.ref());
    ...
    GValue.prototype.set = function (value) {
        ...
        switch (gtype) {
            ...
            case GTYPES.VipsBlob:
                // dup into a fresh memory area we can give to libvips
                var ptr = libglib.g_memdup(value, value.length);

                vips.n_g_free += 1;
                libvips.vips_value_set_blob(this.buffer.ref(),
                    g_free, ptr, value.length);
                break;
                ...
                switch (fundamental) {
                    ...
                    case GTYPES.GObject:
                        if (!(value instanceof vips.GObject)) {
                            throw new Error('can\'t set an object value with ' +
                                value);
                        }
                        libgobject.g_value_set_object(this.buffer.ref(), 
                            value.buffer);
                        break;
